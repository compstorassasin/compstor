...
class Q19 extends TpchQuery {
  ...
  override def execute(ss: SparkSession, tpchTableProvider: TpchTableProvider, iDirPrefix: String): DataFrame = {
    ...
    if (iDirPrefix != "") {
      val fslineitem = lineitem.filter(($"l_shipmode" === "AIR" || $"l_shipmode" === "AIR REG") &&
        $"l_shipinstruct" === "DELIVER IN PERSON").select($"l_partkey", $"l_quantity", $"l_extendedprice", $"l_discount")
      val fspart = part.filter($"p_size" >= 1).select($"p_partkey", $"p_brand", $"p_container", $"p_size")
